'''
    This tutorial shows how to develop an simple facebook bot that replies that
    users type
'''
...
def handle_verification():
    ...
    if request.args.get('hub.verify_token', '') == '123456789':
...
# This part technically clean up received data to pass only meaningful data to
...
def messaging_events(payload):
    ...
    for event in messaging_events:
        ...
        # Pure text message
        if "message" in event \
                and "text" in event["message"] \
                and "quick_reply" not in event["message"]:
            data = event["message"]["text"].encode('unicode_escape')
            yield sender_id, {'type':'text',
                              'data': data,
        ...
        # Quick reply message type
            ...
            yield sender_id, {'type':'quick_reply',
                              'data': data,
                              'message_id': event['message']['mid']}
